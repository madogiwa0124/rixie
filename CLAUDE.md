...
| `Input` | Pure module. `normalize` validates/canonicalizes `Session#chat`/`#live` `user_input` at the boundary (String passthrough; Array content blocks → string-keyed Hashes). |
...
**Multimodal `user_input` uses a Rixie unified content format, normalized at the input boundary and translated in the adapter.**
`user_input` (to `Session#chat` / `Session#live`) is `String | Array<Hash>`. A `String` is plain text (unchanged). An `Array` carries Rixie unified content blocks — `{ type: "text", text: }` and `{ type: "image", source: { type: "base64", media_type:, data: } }`. `Session#chat`/`#live` pass it through `Rixie::Input.normalize` **once at the boundary**: a `String` is returned unchanged (the 99% path stays a cheap String — readable in logs/stores, and OpenAI-compatible endpoints prefer `content: "str"` for text-only); an `Array` is **validated and canonicalized to string-keyed Hashes**. Callers may pass symbol or string keys (CLI-built blocks vs. blocks round-tripped through a JSON store); `Input` accepts both and emits one canonical string-keyed form, so every downstream consumer — `PromptBuilder`, `Message::User#content` (type widened from `String` to `String | Array<Hash>`), the JSON stores, and context replay — sees a single representation. The normalized content flows unchanged through `Session → Task → Run → PromptBuilder`.

Validation lives in `Input`, **not** the adapter (see [adapter.md](.claude/rules/adapter.md)): "which content types Rixie supports" is domain knowledge, so centralizing it at the boundary means a new adapter (Anthropic, …) never re-implements the checks — the inverse would be a `FileSandbox`-style divergence. `Input` validates text and image blocks **uniformly** (a text block must carry a String `text`; an image `source` must be a complete base64 source) and raises `Rixie::InvalidContentError` (a terminal caller-input error, not `LLM::Error`) on an unknown `type:`, a non-Hash block, or an incomplete image source. The adapter is then **pure wire translation**: `OpenAI#encode_content_block` reads only string keys and maps text → `text` / image → `image_url` with a `data:` URI; a block it cannot map is an internal-invariant violation (defensive `InvalidContentError`), never user input. Because both fresh input (normalized to string keys) and store-replayed content (string keys from JSON) are string-keyed, the adapter needs no symbol/string key tolerance. `Session#compress!` flattens an `Array` user content to text for the summary (text inline, image → `[image]` placeholder) rather than inspecting the Hash.

Passing each provider's *native* content format directly through was rejected: it would make `user_input` provider-specific, breaking the provider-independence that the `model:`/`provider:` separation exists to preserve. Consistent with `Tool#input_schema`, no value objects / resolver / DSL are introduced — `user_input` stays plain data, and `Input.normalize` is a pure module function. Full internal `String → Hash` normalization was deliberately **not** adopted (String stays String). Scope is base64 images only; image URLs, PDFs, audio/video, and Vercel-AI-SDK-style `{ type:"image", image: <bytes|dataURL|url> }` input are out of scope.
