...
`Message::User#content` is `String | Array<Hash>`. A `String` is plain text. An `Array` carries Rixie unified content blocks — `{ type: "text", text: }` and `{ type: "image", source: { type: "base64", media_type:, data: } }` — and the adapter is responsible for translating each block to the provider's wire format (e.g. OpenAI encodes an image as `{ type: "image_url", image_url: { url: "data:#{media_type};base64,#{data}" } }`). Other message types always carry `String` content.

**Content blocks are validated and canonicalized upstream, not in the adapter.** `Rixie::Input.normalize` (called at the `Session#chat`/`#live` boundary) validates each block and rewrites it to a canonical **string-keyed** Hash. So the adapter receives only string keys (no symbol/string tolerance needed) and may assume well-formed blocks — it is **pure wire translation**. Do **not** re-validate "which content types Rixie supports" in the adapter: that domain knowledge belongs at the input boundary, and duplicating it across adapters is the mistake `Input` centralization exists to prevent. A block the adapter still cannot map is an internal-invariant violation; a terse defensive `Rixie::InvalidContentError` is acceptable, but it is a safety net, not the validation path. `Rixie::InvalidContentError` is a terminal caller-input error, **not** `LLM::Error` (which signals a possibly-transient provider failure).
